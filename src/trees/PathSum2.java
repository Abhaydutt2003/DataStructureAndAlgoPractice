...
public class PathSum2 {
    ...
    public static void util2(TreeNode root, int target, List<Integer> nodesTillNow, int sumTillNow) {
        if (root == null) {
            return;
        }
        nodesTillNow.add(root.val);
        sumTillNow += root.val;
        if (sumTillNow == target && (root.left == null && root.right == null)) {
            List<Integer> toPut = new ArrayList<Integer>(nodesTillNow);
            ans.add(toPut);
        }
        util2(root.left, target, nodesTillNow, sumTillNow);
        util2(root.right, target, nodesTillNow, sumTillNow);
        nodesTillNow.remove(nodesTillNow.size() - 1);
    }
